#include "flexlib/reflect/TypeInfo.hpp" // IWYU pragma: associated
...
class TypeUnwrapper
  : public clang::TypeVisitor<TypeUnwrapper, bool>
{
public:
  TypeUnwrapper(TypeInfo* targetType
  ...
  bool VisitBuiltinType(const clang::BuiltinType* tp)
  {
    ...
    m_targetType->m_scopedName
      = m_targetType->m_declaredName;

    m_targetType->m_fullQualifiedName
      = m_targetType->m_declaredName;

    DVLOG(11)
      << "VisitBuiltinType done...";
  ...
  bool VisitTemplateSpecializationType(
    const clang::TemplateSpecializationType* tp)
  {
    DVLOG(11)
    ...
    if (tplName.getKind() != clang::TemplateName::Template) {
      DVLOG(11)
        << "VisitTemplateSpecializationType skipped...";
      return false;
    }

    const clang::TemplateDecl* tplDecl
      = tplName.getAsTemplateDecl();
    DCHECK(tplDecl);
    result.decl = tplDecl;
    ...
    if (tp->isTypeAlias()) {
      result.aliasedType
        = TypeInfo::Create(tp->getAliasedType(), m_astContext);
    }

    DCHECK(tp);
    for (const clang::TemplateArgument& tplArg
         : tp->template_arguments())
    {
      ...
      switch (tplArg.getKind())
      {
        case clang::TemplateArgument::Null: {
          argInfo
            = TemplateType::GenericArg{
                TemplateType::NullTplArg};
          break;
        }
        case clang::TemplateArgument::Type: {
          DCHECK(m_astContext);
          argInfo
            = TypeInfo::Create(tplArg.getAsType(), m_astContext);
          break;
        }
        ...
        case clang::TemplateArgument::NullPtr: {
          argInfo
            = TemplateType::GenericArg{
                TemplateType::NullPtrTplArg};
          break;
        }
        case clang::TemplateArgument::Integral: {
          argInfo
            = ConvertAPSInt(tplArg.getAsIntegral()).AsSigned();
          break;
        }
        case clang::TemplateArgument::Template:{
          argInfo
            = TemplateType::GenericArg{
                TemplateType::TemplateTplArg};
          break;
        }
        case clang::TemplateArgument::TemplateExpansion: {
          argInfo
            = TemplateType::GenericArg{
                TemplateType::TemplateExpansionTplArg};
          break;
        }
        ...
        case clang::TemplateArgument::Pack: {
          argInfo
            = TemplateType::GenericArg{TemplateType::PackTplArg};
          break;
        }
      }
    ...
    if (isWellKnownType) {
      ...
      m_targetType->m_type
        = static_cast<TemplateType&&>(std::move(result));
    }

    DVLOG(11)
      << "VisitTemplateSpecializationType done...";
    return true;
  }

  bool VisitTemplateTypeParmType(
    const clang::TemplateTypeParmType* tp)
  {
    DVLOG(11)
  ...
  bool DetectWellKnownType(
    WellKnownType& typeInfo)
  {
    DVLOG(11)
      << "DetectWellKnownType...";

    bool result = true;
    if (m_targetType->m_fullQualifiedName
      == "std::basic_string")
    {
      const BuiltinType* charType
        = typeInfo.GetTypeArg(0)->getAsBuiltin();
      DCHECK(charType);
    ...
    else if (m_targetType->m_fullQualifiedName
      == "std::vector")
    {
    ...
    DVLOG(11)
      << "DetectWellKnownType done...";

    return result;
  }

  bool VisitEnumType(
    const clang::EnumType* tp)
  {
    DVLOG(11)
      << "VisitEnumType...";
  ...
  bool VisitConstantArrayType(
    const clang::ConstantArrayType* tp)
  {
    DVLOG(11)
    ...
    TypeInfoPtr itemType
      = TypeInfo::Create(tp->getElementType(), m_astContext);

    if (itemType->getAsArrayType() != nullptr)
    {
      const ArrayType* subArray
        = itemType->getAsArrayType();
...
private:
  void FillDeclDependentFields(
    const clang::NamedDecl* decl)
  {
    DVLOG(11)
      << "FillDeclDependentFields...";

    DCHECK(m_targetType);

    NamedDeclInfo declInfo;

    AstReflector::SetupNamedDeclInfo(
      decl, &declInfo, m_astContext);

    m_targetType->m_declaredName
      = declInfo.name;

    m_targetType->m_scopedName
      = declInfo.scopeSpecifier.empty()
        ? declInfo.name
        : declInfo.scopeSpecifier
          + "::"
          + declInfo.name;

    m_targetType->m_fullQualifiedName
      = declInfo.namespaceQualifier.empty()
        ? m_targetType->m_scopedName
        : declInfo.namespaceQualifier
          + "::"
          + m_targetType->m_scopedName;
  }
...
TypeInfo::TypeDescr TypeInfo::getTypeDescr() const
{
  ...
  result.scopeSpec
    = getScope(m_scopedName, m_declaredName);
  result.namespaceQual
    = getScope(m_fullQualifiedName, m_scopedName);
...
TypeInfoPtr TypeInfo::Create(
  const clang::QualType& qt
  , const clang::ASTContext* astContext)
{
  DCHECK(astContext);
  DVLOG(11)
    << "TypeInfo::Create for QualType...";
  ...
  if (!unwrapped) // result->m_type.empty()) /// \todo
  {
    DCHECK(result->m_typeDecl);
    result->m_typeDecl->dump();
  }

  DVLOG(11)
    << "TypeInfo::Create for QualType done...";

  return result;
}

TypeInfoPtr TypeInfo::Create(
  const TypeInfo::TypeDescr& descr)
{
  DVLOG(11)
    << "TypeInfo::Create for TypeDescr...";
  ...
  result->m_scopedName
    = descr.scopeSpec.empty()
      ? descr.name
      : descr.scopeSpec + "::" + descr.name;

  result->m_fullQualifiedName
    = descr.namespaceQual.empty()
      ? result->m_scopedName
      : descr.namespaceQual + "::" + result->m_scopedName;
